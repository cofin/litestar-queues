...
class CockroachPsycopgSyncQueueStore(SQLSpecQueueStore):
    ...
    def drop_statements(self) -> list[str]:
        """Return statements that drop cockroach_psycopg sync queue artifacts."""
        if not self._manage_schema:
            return []
        return [
...
class CockroachPsycopgAsyncQueueStore(SQLSpecQueueStore):
    ...
    def drop_statements(self) -> list[str]:
        """Return statements that drop cockroach_psycopg async queue artifacts."""
        if not self._manage_schema:
            return []
        return [
            self._to_sql(sql.drop_index(self._index_name("heartbeat")).if_exists()),
