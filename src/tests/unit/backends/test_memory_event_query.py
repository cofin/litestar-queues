"""Memory event-log query, summary, and filtered retention."""
...
async def _log(*events: QueueEvent) -> InMemoryQueueEventLog:
    log = InMemoryQueueEventLog(EventHistoryConfig())
    for event in events:
        await log.publish_event(event)
    return log


async def test_query_filters_on_every_dimension() -> None:
    log = await _log(
...
async def test_query_orders_and_pages_stably() -> None:
    log = await _log(*[_event(str(index), offset=index) for index in range(5)])
...
async def test_summaries_are_scoped_and_rank_levels() -> None:
    log = await _log(
...
async def test_summary_rejects_pagination() -> None:
    log = await _log(_event("a", offset=0))

    with pytest.raises(QueueConfigurationError):
        await log.summarize_stages(QueueEventQuery(limit=1))


async def test_filtered_bounded_cleanup_converges_and_spares_unmatched() -> None:
    log = await _log(
...
async def test_exclude_implements_first_match_wins() -> None:
    log = await _log(
        _event("a", offset=0, scope_key="acme", level="error"), _event("b", offset=1, scope_key="acme", level="info")
    )
    cutoff = BASE + timedelta(hours=1)
