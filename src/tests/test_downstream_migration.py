...
async def test_downstream_style_schedules_preserve_task_metadata_and_jitter(monkeypatch):
    task_module = import_module("litestar_queues.task")
    monkeypatch.setattr(task_module.random, "uniform", lambda lower, upper: upper)
    ...
    assert distributed_record.metadata["schedule"]["interval"] == 300.0
    assert distributed_record.metadata["schedule"]["initial_delay"] == 60.0
    assert distributed_record.metadata["schedule"]["jitter"] == 30.0
...
async def test_initialize_schedules_replaces_changed_schedule_definition() -> None:
    ...
    assert active_record.metadata["schedule"]["interval"] == 300.0
