...
This standalone app runs a Litestar page with Advanced Alchemy queue persistence through sqlite+aiosqlite plus memory Channels delivery.
...
The queue database defaults to queue-advanced-alchemy.db under the example directory. The demo enables create_all=True and create_schema=True so the local schema is auto-created for a copyable app.
