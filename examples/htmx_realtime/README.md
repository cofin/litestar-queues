# HTMX Realtime Queue Events (Memory Backend)

This standalone app runs a Litestar page with the default memory queue backend,
the default local worker behavior, and plugin-owned queue event streams in one
process. The first viewport is an animated crawl fed by task events. The second
...
LITESTAR_APP=examples.htmx_realtime.app:app \
...
Open `http://127.0.0.1:8000`, choose WebSocket or SSE, and press Restart.

## What It Demonstrates

- `HTMXPlugin()` with `litestar-vite` `mode="htmx"`.
- The Litestar Vite HTMX helper registered with `registerHtmxExtension()`.
- HTMX 2 indicators, disabled elements, sync replacement, transition swaps, and
  `HX-Trigger-After-Swap` events through `HTMXTemplate`.
