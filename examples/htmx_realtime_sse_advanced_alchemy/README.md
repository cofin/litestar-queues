...
the code. Queue persistence runs through Advanced Alchemy with
...
- **One small JS adapter.** Queue events are JSON, so the extension cannot swap
  them as HTML. `../shared/resources/main-sse.ts` listens for `htmx:sseBeforeMessage`, parses
  ...
  the backend.
- **htmx wiring.** `../shared/resources/main-sse.ts` imports htmx, publishes it as
