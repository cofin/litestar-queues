...
This standalone app runs a Litestar page with SQLSpec queue persistence through Aiosqlite plus memory Channels delivery.
...
This copy is WebSocket-only. It uses the plugin-owned `/queues/events/tasks/{task_id}` and `/queues/events/custom/{scope_key}`
...
The queue database defaults to queue-sqlspec.db under the example directory. SQLSpecBackendConfig sets create_schema=False and run_migrations=True, so startup calls the SQLSpec migration path for the local queue schema.
